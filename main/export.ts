...
export default class Export extends (EventEmitter as new () => TypedEventEmitter<ExportEvents>) {
  ...
  private readonly _start = PCancelable.fn(
    async (onCancel: OnCancelFunction) => {
      ...
      try {
        ...
        this.text = i18n.t("ExportCompleted");
  ...
  get canPreviewExport() {
    return (
      [Format.gif, Format.apng].includes(this.format) &&
      this.finalFilePath !== undefined
    );
  }
  ...
  get finalFilePath() {
    ...
    return (
      filePath && ((this.options.extras.targetFilePath as string) ?? filePath)
    );
  }
  ...
  filePath = async ({ fileType }: { fileType?: Format } = {}) => {
    ...
    this.conversion = Conversion.getOrCreate(
      this.video,
      format,
  ...
  cancel = () => {
    ...
    this.text = i18n.t("ExportCanceled");
    this.context.isCanceled = true;
    this.emit("updated", this.data);
  };
  ...
  private readonly captureError = (error: Error, fromConversion = false) => {
    if ((error as CancelError).isCanceled) {
      ...
      if (!this.error) {
        ...
        showError(
          error,
          fromConversion ? undefined : { plugin: this.options.plugin }
        );
      }
    }
  ...
  private readonly captureConversionError = (error: Error) =>
...
export const setUpExportsListeners = () => {
  ipcMain.on("drag-export", async (event: any, id: string) => {
    ...
    if (
      conversion &&
      (await conversion.filePathExists()) &&
      exportMap?.status === ExportStatus.completed
    ) {
    ...
    if (Export.all.some((exp) => exp.status === ExportStatus.inProgress)) {
      ...
      ensureDockIsShowingSync(() => {
        const buttonIndex = dialog.showMessageBoxSync({
          ...
          defaultId: 0,
          cancelId: 1,
          message: i18n.t('BeforeQuitExportMessage'),
          detail: i18n.t('BeforeQuitExportDetail'),
        });
