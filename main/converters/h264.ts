...
const convertToGif = PCancelable.fn(
  async (options: ConvertOptions, onCancel: PCancelable.OnCancelFunction) => {
    const palettePath = tempy.file({ extension: "png" });

    const paletteProcess = convert(
      palettePath,
      { shouldTrack: false },
      conditionalArgs(
        "-i",
    ...
    const conversionProcess = convert(
      options.outputPath,
      {
      ...
      conditionalArgs(
        ...
        "-loop",
        shouldLoop ? "0" : "-1", // 0 == forever; -1 == no loop
        {
          args: [
            "-ss",
            options.startTime.toString(),
    ...
    const compressProcess = compress(
      options.outputPath,
      {
        onProgress: (progress, estimate) => {
...
const convertToWebm = (options: ConvertOptions) =>
  convert(
    ...
    conditionalArgs(
      ...
      "-threads",
      Math.max(os.cpus().length - 1, 1).toString(),
      "-deadline",
      "good", // `best` is twice as slow and only slighty better
      "-b:v",
      "1M", // Bitrate (same as the MP4)
      "-codec:v",
      "vp9",
      "-codec:a",
      "vorbis",
      "-ac",
      "2", // https://stackoverflow.com/questions/19004762/ffmpeg-covert-from-mp4-to-webm-only-working-on-some-files
      "-strict",
      "-2", // Needed because `vorbis` is experimental
      "-r",
      options.fps.toString(),
      {
        args: ["-an"],
...
const convertToApng = (options: ConvertOptions) =>
  convert(
    ...
    conditionalArgs(
      ...
      "-plays",
      settings.get("loopExports") ? "0" : "1", // 0 == forever; 1 == no loop
      {
        args: ["-an"],
        if: options.shouldMute,
      },
...
// eslint-disable-next-line @typescript-eslint/promise-function-async
export const crop = (options: ConvertOptions) =>
  convert(
    options.outputPath,
    {
      onProgress: (progress, estimate) => {
