...
export type RemoteStateHook<Base extends RemoteState> =
  Base extends RemoteState<infer State, infer Actions>
    ? Actions & {
        state: State;
        isLoading: boolean;
        refreshState: () => void;
...
export type RemoteStateHandler<Base extends RemoteState> =
  Base extends RemoteState<infer State, infer Actions>
    ? (sendUpdate: (state: State, id?: string) => void) => {
        actions: {
          [Key in keyof Actions]: Actions[Key] extends (...args: any[]) => any
            ? (id: string, ...args: Parameters<Actions[Key]>) => void
            : never;
        };
...
export type EditorOptionsRemoteState = RemoteState<
  ExportOptions,
  {
    updatePluginUsage: ({
      format,
    ...
    updateFpsUsage: ({ format, fps }: { format: Format; fps: number }) => void;
  }
>;
...
export type ExportsRemoteState = RemoteState<
  ExportState,
  {
    copy: () => void;
    cancel: () => void;
